...
function insert($table,$data){
    include('../../src/config/connection.php');
    include('../../src/config/immage_upload.php');
...
if(sizeof($data["unique_field"])>0){
    ...
    $sql = "INSERT INTO `$table` (`$matstring_field`) VALUES ('$matstring')"; 
        ...
        if(!empty($data["upload_immage"])){
            foreach($data["upload_immage"] as $key => $up_img){
                $path=$table."_".$up_img['name']."/";
                if(file_exists($path) && $_FILES[$up_img["name"]["name"]]!=NULL){
                    $ext=resizeUpload($_FILES[$up_img["name"]],$path,$last_id,$up_img["weidth"],$up_img["height"]); 
                }else{
                     mkdir($path);
                     $ext=resizeUpload($_FILES[$up_img["name"]],$path,$last_id,$up_img["weidth"],$up_img["height"]);
                }
                if($ext){
                    $db_field=$up_img["name"]; 
                    $datas=[];
                    
                    if(!isset($up_img["weidth"]) || !isset($up_img["height"])){
                        $db_name_save=$path."original_".$last_id;
                    }else{
                        $db_name_save=$path.$last_id;
                    }
                    
                    
                    $datas=array(
                                "data"=>array(
                                        $db_field=>$db_name_save.".".$ext
                                    ));
                    $update_data=update($table,$datas,$last_id);
                    unset($datas);
                }
            } 
        }
        ...
        if(!empty($data["file"])){
            foreach($data["file"] as $key => $up_img){
                $path=$table."_".$up_img['name']."/";
                $ext = pathinfo($_FILES[$up_img["name"]]['name'], PATHINFO_EXTENSION);
                if(file_exists($path)){
                    move_uploaded_file($_FILES[$up_img["name"]]['tmp_name'],"$path/$last_id.$ext"); 
                }else{
                    mkdir($path);
                    move_uploaded_file($_FILES[$up_img["name"]]['tmp_name'],"$path/$last_id.$ext"); 
                }
