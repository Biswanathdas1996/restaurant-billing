...
if(isset($_GET["search"]) && $_GET["search"] !=null){
    ...
    $menu_type=$_GET["menu_type"];
     ...
     $sql = "SELECT * FROM food_demo WHERE menu_type = '$menu_type' AND title LIKE '%$search%'";  
     // $sql = "SELECT * FROM food_demo WHERE menu_type = '$menu_type' AND MATCH(title) AGAINST('$search')";  
                    ...
                    if(mysqli_num_rows($retval) > 0){  
                     while($row = mysqli_fetch_assoc($retval)){ 
                              ...
                              //  $item_temp_data["category"]=$get_category[0]["name"];
