 ...
 for($i=1;$i<=$total_no_of_working_table=$get_all_tables[0]['total_working_no_of_table'];$i++){
    ...
    $get_scan_data=select('food_scan_report',[
     ...
     if(!empty($check_table_status)){
                 $button_class='class="btn btn-danger modalltext"  title="Ongoing Order"  value="'.$check_table_status[0]['id'].'"';
                 $order_id='Order:<b>'.$check_table_status[0]['id'].'</b>';
     }else{
            ...
            if(!empty($get_scan_data)){
                ...
                $button_class='class="btn btn-success table_reserve tableId" title="Free Table"  data="'."Parcel-".$i.'"';
                $order_id='';
            } 
     ...
     $temp_div_html='<div class="col-md-2 col-sm-6 col-xs-6" style="padding: 0px 0px;">
                   <button type="button" '.$button_class.'  value="" style="padding: 8px 6px; width:95%; margin:2px;">
                          <span class="" style="font-size: 20px;">Parcel-'.$i.'</span>
                         <p style="margin: 0px 0px;padding-top: 5px;font-size: 11px;font-weight:300">&nbsp;&nbsp;'.$order_id.'</p>
